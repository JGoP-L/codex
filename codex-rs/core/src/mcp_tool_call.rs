...
fn openai_file_input_params_for_server(
    ...
    Some(OpenAIFileInputParams {
        names,
        store_in_oai_library: declared_openai_file_upload_store_in_oai_library(meta, input_schema),
    })
}
...
fn declared_openai_file_upload_store_in_oai_library(
    ...
    // Hidden metadata only opts the tool into host plumbing. Durable Library storage must
    // still be visible as the exact `store_in_oai_library: true` arg Guardian reviews.
    tool_opted_in
        && visible_required_true_input_schema_arg(
            input_schema,
            MCP_TOOL_OPENAI_FILE_UPLOAD_STORE_IN_OAI_LIBRARY_KEY,
        )
}
